...
public class TextFileViewerViewModel : ViewModelBase {
	...
	public TextFileViewerViewModel(TextFileViewerModel textFileViewerModel, HighlightService highlightService) {
		...
		this.Lines = this._textFileViewerModel.Lines.Select(x => x?.Select(highlightService.HighlightText).ToArray() ?? []).ToReadOnlyReactiveProperty([]).AddTo(this.CompositeDisposable);
	...
	public ReadOnlyReactiveProperty<HighlightedTextLine[]> Lines {
		get;
	}
